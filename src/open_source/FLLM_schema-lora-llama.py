...
args.device_list = ["cuda:5", "cuda:4", "cuda:3", "cuda:2", "cuda:1", "cpu"]
...
for i, task_description in enumerate(training_tasks):
    with torch.no_grad():
        ...
        while len(auto_stack) > 0:
            ...
            tool_cnt = 0
            prompt = f'### Human: You will help me generate a plan for the Problem: "{task_description}" by answering a series of my questions.\n'
            ...
            prompt_ids = [s.replace("<pad>", "").replace("</s>", "") for s in seqGen.tokenizer.batch_decode(input_ids)]
            logging.info(output_sequence[0][len(prompt_ids[0]):])
            
            scores = output['scores']
            log_probs = []
            for t in range(1, tool_cnt + 1):
                out_id = torch.tensor([seqGen.tokenizer.encode(prompt + str(t))])
                logprob = 0
                length = out_id.size(-1)
                start_position = input_ids.size(-1)
                if len(scores) >= length - start_position:
                    for l in range(start_position, length):
                        score = scores[l - start_position][0]
...
for e in range(epochs):
    ...
    for i, task_description in enumerate(training_tasks):
        ...
        with torch.no_grad():
            ...
            while len(auto_stack) > 0:
                ...
                for t in range(1, tool_cnt + 1):
                    ...
                    length = out_id.size(-1)
                    start_position = input_ids.size(-1)
                    print(str(out_id[0][start_position:length]), str(seqGen.tokenizer.decode(out_id[0][start_position:])))
                    if len(scores) >= length - start_position:
                        for l in range(start_position, length):
                            score = scores[l - start_position][0]
                            logprob += score[out_id[0][l]]
                            if seqGen.tokenizer.decode(out_id[0][l]) == "</s>":
                                break
                            if l == length - 1 and len(scores) > length - start_position and seqGen.tokenizer.decode(out_id[0][l]) == "</s>":
                                score = scores[length - start_position][0]
                                logprob += score[seqGen.tokenizer.get_vocab()['</s>']]
                    log_probs.append(logprob)
                logging.info(log_probs)
            ...
            if len(auto_stack) > 0:
                ...
                with torch.no_grad():
                    ...
                    seqCombination.close_module_seq()
                    loss_coef = (ave_task_reward - baseline[i])
                    if ave_task_reward - baseline[i] > 0.0:
                        loss_coef *= args.pos_reward_coef
        ...
        while len(auto_stack) > 0:
            ...
            loss = total_log_prob * loss_coef
            loss.backward()
            epoch_total_loss += loss.detach()
            
            choice = choice.item() + 1
            logging.info(str(choice) + str(total_log_prob))
        ...
        module_list = action[::-1]
        logging.info(f'Module Sequence: {module_list}')
        baseline[i] -= (1 - args.bl_dec) * (-loss_coef)
